...
// BMI計算
const countBMI = function(weight, height) {return weight / (height * height);}
...
// 計算BMI
window.onload = function() {
  document.querySelector("#countBMI_btn").addEventListener("click", function() {
    const height = parseFloat(document.querySelector("#countBMI").value) / 100; // 公分轉公尺
    const weight = parseFloat(document.querySelector("#countBMI2").value);
    let bmiResult = countBMI(weight, height);
    document.querySelector("#bmi_answer").innerHTML = "BMI: " + bmiResult.toFixed(2);
  });
}
